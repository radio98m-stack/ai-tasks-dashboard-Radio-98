...
# Founder admin bootstrap variables
...
def bootstrap_founder_admin():
    ...
    if not (SUPABASE_SERVICE_ROLE_KEY and FOUNDER_ADMIN_EMAIL and FOUNDER_ADMIN_PASSWORD):
        st.warning("Bootstrap admin is enabled but missing required environment variables.")
        return
    try:
        ...
        if res.get('error'):
            ...
            st.success("Founder admin account created. You can now log in.")
    except Exception as e:
